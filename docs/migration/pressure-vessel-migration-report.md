...
- `NozzleCalcInput` / `NozzleCalcResult` / `UG45Table` are now available from provided source and reflected in core nozzle contracts.

## Additional source files required next
To complete a formula-accurate migration, the following are required:

1. Final parity cross-check between core nozzle contract naming and legacy member naming (cosmetic/non-formula)
