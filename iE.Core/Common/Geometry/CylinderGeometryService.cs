...
{
    public class CylinderGeometryService
    {
        public CylinderGeometryResult Calculate(CylinderGeometryInput input)
        {
            if (input.Radius <= 0)
            ...
            double lateralSurfaceArea = 2.0 * Math.PI * input.Radius * input.Height;
            double totalSurfaceArea = 2.0 * Math.PI * input.Radius * (input.Radius + input.Height);
            double volume = Math.PI * input.Radius * input.Radius * input.Height;

            return new CylinderGeometryResult
            {
