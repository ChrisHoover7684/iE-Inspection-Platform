...
public class PressureVesselServicesTests
{
    ...
    public void CylindricalShell_CalculatesCircAndLongAndGoverning()
    {
        ...
        var input = new CylindricalShellInput(150, 24, 20000, 1.0, 0.125, 0.5);
    ...
    {
        var service = new ShellThicknessService();
        var input = new SphericalShellInput(150, 24, 20000, 1.0, 0.125, 0.5);
    ...
    public void Nozzle_Ug45_GoverningThicknessCalculated()
    {
        ...
        var input = new NozzleThicknessInput(150, 20000, 1.0, 2, 0.0625, 0.25, 0.1, 0.08, 0.109);

        var result = service.Calculate(input);

        Assert.True(result.GoverningRequiredThicknessIn > 0);
        Assert.Equal(Math.Max(result.TaRequiredThicknessIn, result.TbRequiredThicknessIn), result.GoverningRequiredThicknessIn, 6);
    }
}
