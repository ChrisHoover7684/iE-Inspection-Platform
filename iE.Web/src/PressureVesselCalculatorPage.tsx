...
export function PressureVesselCalculatorPage() {
  ...
  return <div style={{ padding: 16 }}><h1>Pressure Vessel Calculator</h1>{error && <div style={{ color: 'crimson' }}>{error}</div>}
    ...
    {tab === 'nozzles' && <form onSubmit={async e=>{e.preventDefault();try{{ const r = await pressureVesselApi.calculateNozzle({ input: noz, materialStress: null }); setNozResult(r.result); }setError(null);}catch(e){handleErr(e)}}}>
