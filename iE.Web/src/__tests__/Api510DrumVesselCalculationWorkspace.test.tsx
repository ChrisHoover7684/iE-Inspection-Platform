...
describe("Api510DrumVesselCalculationWorkspace", () => {
  it("drum shell lookup uses shell material", async () => {
    const spy = vi
    ...
    render(
      <Api510DrumVesselCalculationWorkspace
        fieldValues={{
          "api510.external.drum-vessel.design-pressure": 100,
          "api510.external.drum-vessel.design-temperature": 600,
        }}
      />,
    );
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({
        materialSpec: "SA-516",
        designTemperatureF: 600,
      }),
    );
  });

  it("drum head and nozzle-head lookup use head material", async () => {
    const spy = vi
      .spyOn(pressureVesselApi, "resolveMaterialStress")
      .mockResolvedValueOnce({
        isValid: true,
        allowableStressPsi: 18000,
        materialMatched: "head",
        temperatureUsed: 600,
        wasInterpolated: false,
        wasExtrapolated: false,
        message: "ok",
    ...
    fireEvent.change(screen.getByLabelText("Head Material Specification"), {
      target: { value: "SA-516H" },
    });
    ...
    expect(spy).toHaveBeenLastCalledWith(
      expect.objectContaining({
        materialSpec: "SA-516H",
        designTemperatureF: 600,
      }),
    );
  });

  it("drum nozzle-shell lookup uses shell material", async () => {
    const spy = vi
      .spyOn(pressureVesselApi, "resolveMaterialStress")
      .mockResolvedValueOnce({
        isValid: true,
    ...
    await screen.findByText("Resolved Allowable Stress (psi): 17500");
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ materialSpec: "SA-516S" }),
    );
  });
  ...
  it("reads draft vessel design conditions, subtype, selected components, and head type", () => {
    ...
    expect(screen.getByLabelText("Equipment Subtype")).toHaveValue(
      "Vertical Drum",
    );
    expect(
      screen.getByText(
        "Design Pressure: api510.external.drum-vessel.design-pressure = 300",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "Design Temperature: api510.external.drum-vessel.design-temperature = 650",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: "Draft Context" }),
    ).toHaveTextContent("Insulation");
    fireEvent.change(screen.getByLabelText("Component"), {
      target: { value: "heads" },
    });
    expect(screen.getByLabelText("Head Type")).toHaveValue("Hemispherical");
    expect(
      screen.getByLabelText("Effective Inside Radius (in.)"),
    ).toBeInTheDocument();
  });
  ...
  it("Heads show UG-32 inputs and head type geometry switches", () => {
    ...
    expect(
      screen.getByLabelText("Effective Inside Diameter (in.)"),
    ).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("Head Type"), {
      target: { value: "Hemispherical" },
    });
    expect(
      screen.getByLabelText("Effective Inside Radius (in.)"),
    ).toBeInTheDocument();
    expect(
      screen.queryByLabelText("Effective Inside Diameter (in.)"),
  ...
  it("successful calculation displays snapshot preview and save disabled without report context", async () => {
    vi.mocked(executeApi510ComponentCalculation).mockResolvedValueOnce({
      ...
      resultSummary: "ok",
      warnings: [],
      snapshotReadyPayload: { id: "snap" },
    } as any);
    render(
      <Api510DrumVesselCalculationWorkspace
        fieldValues={{
          "api510.external.drum-vessel.design-pressure": 100,
          "api510.external.drum-vessel.design-temperature": 200,
        }}
      />,
    );
  ...
  it("finding draft includes equipment subtype/component context", async () => {
    ...
    vi.mocked(executeApi510ComponentCalculation).mockResolvedValueOnce({
      ...
      snapshotReadyPayload: {
        id: "s1",
        inputs: { prefill: { equipmentSubtype: "Vertical Drum" } },
      },
    } as any);
    render(
      <Api510DrumVesselCalculationWorkspace
    ...
    fireEvent.change(screen.getByLabelText("Equipment Subtype"), {
      target: { value: "Vertical Drum" },
    });
    await mockResolveAllowableStress();
    fireEvent.click(screen.getByRole("button", { name: "Run Calculation" }));
    ...
    expect(onCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        equipmentSubtype: "Vertical Drum",
        componentKey: "shell",
      }),
    );
  });
...
it("standalone vessel design conditions are passed via panel", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Vessel Design Pressure"), {
    target: { value: "200" },
  });
  fireEvent.change(screen.getByLabelText("Vessel Design Temperature"), {
    target: { value: "600" },
  });
  await mockResolveAllowableStress(16000);
  ...
  expect(vi.mocked(executeApi510ComponentCalculation)).toHaveBeenCalledWith(
    expect.objectContaining({
      prefill: expect.objectContaining({
        designPressureValue: "200",
        designTemperatureValue: "600",
      }),
    }),
  );
});
