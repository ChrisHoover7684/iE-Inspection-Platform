...
import {
  ...
  buildApi510DrumVesselExternalDraftPayload,
  drumVesselSubtypeByReportTypeId,
  isApi510DrumVesselExternalReportType,
  readApi510DrumVesselExternalDraftPayload
} from './api510DrumVesselReportDraft';
import {
...
export { API510_DRUM_VESSEL_EXTERNAL_REPORT_LOCAL_STORAGE_KEY };

type ChecklistField = Api510DrumVesselExternalChecklistField;
type ComponentReportState = Api510DrumVesselExternalComponentState;

const defaultComponentsBySubtype: Record<string, string[]> = {
  'Horizontal Drum': ['Shell', 'Heads', 'Nozzles', 'Saddles / Supports', 'Manway'],
  'Vertical Drum': ['Shell', 'Heads', 'Nozzles', 'Skirt / Legs / Supports', 'Manway'],
  'Separator / KO Drum': ['Shell', 'Heads', 'Nozzles', 'Supports', 'Manway'],
  'Accumulator / Receiver': ['Shell', 'Heads', 'Nozzles', 'Supports'],
  'Generic Pressure Vessel': ['Shell', 'Heads', 'Nozzles', 'Supports', 'Manway']
};
...
const materialFields = [
  ...
  ['headMaterialSpec', 'Head Material Spec'],
  ['headMaterialGrade', 'Head Material Grade'],
  ['headProductForm', 'Head Product Form'],
  ['headType', 'Head Type']
] as const;
...
const equipmentSubtypeFromReportType = (reportTypeId: string | undefined) => isApi510DrumVesselExternalReportType(reportTypeId) ? drumVesselSubtypeByReportTypeId[reportTypeId] : 'Generic Pressure Vessel';
...
const calculationFieldValues = (header: Record<string, string> | undefined): CalculationFieldValuesMap => ({
  'api510.external.drum-vessel.design-pressure': header?.vesselDesignPressure,
  'api510.external.drum-vessel.design-temperature': header?.vesselDesignTemperature
});

export function Api510DrumVesselExternalReportPage() {
  ...
  const [savedDraftPayload, setSavedDraftPayload] = useState<Api510DrumVesselExternalDraftPayload | undefined>(() => readApi510DrumVesselExternalDraftPayload());
  ...
  const defaultComponents = defaultComponentsBySubtype[equipmentSubtype] ?? defaultComponentsBySubtype['Generic Pressure Vessel'];

  const components = useMemo(() => {
    const selected = isApi510DrumVesselExternalReportType(draft?.reportTypeId) ? draft?.components ?? [] : [];
    return Array.from(new Set([...defaultComponents, ...selected]));
  }, [defaultComponents, draft]);
  ...
  const [summaryFields, setSummaryFields] = useState<Api510DrumVesselExternalSummaryFieldsDraft>(() => savedDraftPayload?.summaryFields ?? {
  ...
  const handleSaveLocalDraft = () => {
    ...
    const payload = buildApi510DrumVesselExternalDraftPayload({
      sourceStartWizardDraft: draft,
      sourceStartWizardDraftStorageKey: API_INSPECTION_DRAFT_SETUP_STORAGE_KEY,
      components,
      componentStates,
    ...
    window.localStorage.setItem(API510_DRUM_VESSEL_EXTERNAL_REPORT_LOCAL_STORAGE_KEY, JSON.stringify(payload));
    setSavedDraftPayload(payload);
    setLastSavedAt(savedAt);
  };

  if (draft && !isApi510DrumVesselExternalReportType(draft.reportTypeId)) {
    return (
      <section className="page api510-drum-vessel-external-report">
        <h2>API 510 Drum/Vessel External Report</h2>
        <p role="alert">The saved draft setup is not for an API 510 External Drum/Vessel report.</p>
  ...
  return (
    <section className="page report-page api510-drum-vessel-external-report">
      <header className="api510-report-header-sticky" aria-label="Sticky report header">
        ...
        <div className="api510-header-actions">
          <Link to="/reports/api-510-drum-vessel-workspace">Open calculation workspace route</Link>
      ...
      <div className="api510-report-layout">
        <main className="api510-report-main">
          ...
          <section id="calculations" className="card wizard-setup-card" aria-label="Calculations">
            <h3>Calculations</h3>
            <p>Drum/Vessel calculation workspace is embedded below using draft design field values from this report.</p>
            <Api510DrumVesselCalculationWorkspace
              hasReportContext
              reportId="api510-drum-vessel-external-draft"
              fieldValues={fieldValues}
              initialEquipmentSubtype={equipmentSubtype as 'Horizontal Drum' | 'Vertical Drum' | 'Separator / KO Drum' | 'Accumulator / Receiver' | 'Generic Pressure Vessel'}
              initialHeadType={draft?.header.headType}
        ...
        <aside className="api510-report-sidebar" aria-label="Report sidebar">
          ...
          <section className="card" aria-label="Report Summary Preview">
            ...
            <dl className="api510-summary-list">
              <dt>Selected Components</dt><dd>{components.length}</dd>
          ...
          <section className="card" aria-label="Draft Payload Debug">
            <details>
              ...
              <pre aria-label="API 510 Drum/Vessel draft payload">{JSON.stringify(draftPayloadPreview, null, 2)}</pre>
            </details>
          </section>
