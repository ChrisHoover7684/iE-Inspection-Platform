...
export async function executeApi510ComponentCalculation({ prefill, calculationType, manualInputs, snapshotContext }: ComponentCalculationExecutionInput): Promise<ComponentCalculationExecutionResult> {
  ...
  if (calculationType === 'ug-27-shell-tmin') {
    ...
    if (insideDiameter === undefined && outsideDiameter === undefined) warnings.push('UG-27 requires inside diameter or outside diameter.');
    ...
    const input = {
      ...
      insideDiameterIn: insideDiameter ?? outsideDiameter!,
      outsideDiameterIn: outsideDiameter ?? insideDiameter!,
    ...
    try {
      ...
      const snapshotReadyPayload: InspectionCalculationSnapshot = {
        ...
        outputs: { response, metadata: { equipmentSubtype: prefill.equipmentSubtype, componentKey: prefill.componentKey, componentLabel: prefill.componentLabel, pressureSide: prefill.resolvedPressureSide, calculationType, manualInputsUsed: input } },
        ...snapshotContext
      };
  ...
  if (calculationType === 'ug-45-nozzle-neck-tmin') {
    ...
    const input: NozzleCalculationRequest['input'] = {
      ...
      ug16MinimumThicknessIn: toNumber(manualInputs?.ug16MinimumThicknessIn) ?? 0, nozzleType: 'PipeNozzle', useOdForTa: false, useIdForTa: true,
      outsideDiameterIn: nozzleOutsideDiameter!, insideDiameterIn: nozzleInsideDiameter!, nominalThicknessIn: nominalThickness!,
      originalThicknessIn: originalThickness!, nominalPipeSize: String(nominalPipeSize), ug45TableMinimumThicknessIn: toNumber(manualInputs?.ug45TableMinimumThicknessIn) ?? null
    };
